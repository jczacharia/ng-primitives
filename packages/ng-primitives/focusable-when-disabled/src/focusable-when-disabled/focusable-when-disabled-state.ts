...
export interface NgpFocusableWhenDisabledState {
  /**
  ...
  /**
   * Whether the element should be focusable when disabled.
   *
   * @remarks
  ...
  /**
   * Set the focusable when disabled state of the element.
...
export interface NgpFocusableWhenDisabledProps {
  /**
   * Whether the element is disabled.
   */
  readonly disabled?: Signal<boolean>;
...
export const [
  NgpFocusableWhenDisabledToken,
  ngpFocusableWhenDisabled,
  injectFocusableWhenDisabledState,
  provideFocusableWhenDisabledState,
] = createPrimitive(
  'NgpFocusableWhenDisabled',
  ({
  ...
  }: NgpFocusableWhenDisabledProps): NgpFocusableWhenDisabledState => {
    ...
    if (isButton) {
      attrBinding(element, 'disabled', () => (disabled() && !focusableWhenDisabled() ? '' : null));
    }
    ...
    return {
      ...
      setAriaDisabled,
      setTabIndex,
    } satisfies NgpFocusableWhenDisabledState;
  },
);
