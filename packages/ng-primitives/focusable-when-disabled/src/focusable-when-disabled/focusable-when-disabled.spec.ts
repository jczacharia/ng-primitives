...
import { NgpFocusableWhenDisabled } from './focusable-when-disabled';

@Directive({
  selector: '[ngpInteractive]',
  hostDirectives: [
    {
      directive: NgpButton,
      inputs: ['disabled'],
    ...
    {
      directive: NgpFocusableWhenDisabled,
      inputs: ['disabled', 'ngpFocusableWhenDisabled', 'aria-disabled', 'tabIndex'],
    },
  ],
})
export class NgpInteractive {
  ngpButton = inject(NgpButton);
  ngpFocusableWhenDisabled = inject(NgpFocusableWhenDisabled);

  constructor() {
    effect(() => {
      console.log('ngpButton', this.ngpButton.disabled());
      console.log('ngpFocusableWhenDisabled', this.ngpFocusableWhenDisabled.disabled());
    });
  }
}

describe('NgpFocusableWhenDisabled', () => {
  it('should set the disabled attribute when disabled', async () => {
    const container = await render(`<button ngpInteractive [disabled]="true"></button>`, {
      imports: [NgpInteractive],
    });
  ...
  it('should not set the disabled attribute when not disabled', async () => {
    const container = await render(`<button ngpFocusableWhenDisabled></button>`, {
      imports: [NgpFocusableWhenDisabled],
    });
  ...
  it('should not set the disabled attribute when not a button', async () => {
    const container = await render(`<a ngpFocusableWhenDisabled [disabled]="true"></a>`, {
      imports: [NgpFocusableWhenDisabled],
    });
  ...
  it('should set the data-disabled attribute when disabled', async () => {
    const container = await render(`<button ngpFocusableWhenDisabled [disabled]="true"></button>`, {
      imports: [NgpFocusableWhenDisabled],
    });
  ...
  it('should update the data-disabled attribute when disabled changes', async () => {
    const { getByRole, rerender } = await render(
      `<button ngpFocusableWhenDisabled [disabled]="isDisabled"></button>`,
      {
        imports: [NgpFocusableWhenDisabled],
  ...
  describe('focusableWhenDisabled', () => {
    it('should allow a disabled button to receive focus when focusableWhenDisabled is true', async () => {
      const container = await render(
        `<button ngpFocusableWhenDisabled [disabled]="true" [focusableWhenDisabled]="true"></button>`,
        {
          imports: [NgpFocusableWhenDisabled],
        },
      );
    ...
    it('should set aria-disabled on non-native buttons when focusableWhenDisabled is true', async () => {
      const container = await render(
        `<span ngpFocusableWhenDisabled [disabled]="true" [focusableWhenDisabled]="true"></span>`,
        { imports: [NgpFocusableWhenDisabled] },
      );
    ...
    it('should prevent click events when disabled with focusableWhenDisabled', async () => {
      ...
      const container = await render(
        `<button ngpFocusableWhenDisabled [disabled]="true" [focusableWhenDisabled]="true" (click)="onClick()"></button>`,
        {
          imports: [NgpFocusableWhenDisabled],
    ...
    it('should allow focus and blur events when disabled with focusableWhenDisabled', async () => {
      ...
      const container = await render(
        `<button ngpFocusableWhenDisabled [disabled]="true" [focusableWhenDisabled]="true" (focus)="onFocus()" (blur)="onBlur()"></button>`,
        {
          imports: [NgpFocusableWhenDisabled],
  ...
  describe('role attribute (a11y)', () => {
    it('should add role="button" for non-native elements', async () => {
      const container = await render(`<span ngpFocusableWhenDisabled></span>`, {
        imports: [NgpFocusableWhenDisabled],
      });
    ...
    it('should add role="button" for div elements', async () => {
      const container = await render(`<div ngpFocusableWhenDisabled></div>`, {
        imports: [NgpFocusableWhenDisabled],
      });
    ...
    it('should preserve initial role if already set', async () => {
      const container = await render(`<span ngpFocusableWhenDisabled role="menuitem"></span>`, {
        imports: [NgpFocusableWhenDisabled],
      });
  ...
  describe('aria-disabled attribute (a11y)', () => {
    it('should set aria-disabled on disabled non-native buttons', async () => {
      const container = await render(`<span ngpFocusableWhenDisabled [disabled]="true"></span>`, {
        imports: [NgpFocusableWhenDisabled],
      });
    ...
    it('should not set aria-disabled on native buttons when disabled attr is used', async () => {
      const container = await render(
        `<button ngpFocusableWhenDisabled [disabled]="true"></button>`,
        {
          imports: [NgpFocusableWhenDisabled],
        },
      );
  ...
  describe('tabindex attribute (a11y)', () => {
    ...
    it('should allow explicit tabindex override on native buttons', async () => {
      const container = await render(`<button ngpFocusableWhenDisabled [tabIndex]="-1"></button>`, {
        imports: [NgpFocusableWhenDisabled],
      });
  ...
  describe('anchor elements', () => {
    it('should add role="button" for anchor without href', async () => {
      const container = await render(`<a ngpFocusableWhenDisabled></a>`, {
        imports: [NgpFocusableWhenDisabled],
      });
    ...
    it('should not add role="button" for valid links (anchor with href)', async () => {
      const container = await render(
        `<a ngpFocusableWhenDisabled href="https://example.com"></a>`,
        {
          imports: [NgpFocusableWhenDisabled],
        },
      );
    ...
    it('should not add role="button" for anchor with routerLink', async () => {
      const container = await render(`<a ngpFocusableWhenDisabled [routerLink]="['/test']"></a>`, {
        imports: [NgpFocusableWhenDisabled, RouterLink],
      });
    ...
    it('should not trigger synthetic click on Enter for valid links (anchor with href)', async () => {
      const handleClick = jest.fn();
      const container = await render(
        `<a ngpFocusableWhenDisabled href="https://example.com" (click)="onClick()"></a>`,
        {
          imports: [NgpFocusableWhenDisabled],
          componentProperties: { onClick: handleClick },
        },
    ...
    it('should trigger synthetic click on Enter for anchor without href', async () => {
      const handleClick = jest.fn();
      const container = await render(`<a ngpFocusableWhenDisabled (click)="onClick()"></a>`, {
        imports: [NgpFocusableWhenDisabled],
        componentProperties: { onClick: handleClick },
      });
  ...
  describe('click prevention when disabled', () => {
    it('should prevent click on disabled native button', async () => {
      const handleClick = jest.fn();
      const container = await render(
        `<button ngpFocusableWhenDisabled [disabled]="true" (click)="onClick()"></button>`,
        {
          imports: [NgpFocusableWhenDisabled],
          componentProperties: { onClick: handleClick },
        },
    ...
    it('should prevent click on disabled non-native button', async () => {
      const handleClick = jest.fn();
      const container = await render(
        `<span ngpFocusableWhenDisabled [disabled]="true" (click)="onClick()"></span>`,
        {
          imports: [NgpFocusableWhenDisabled],
          componentProperties: { onClick: handleClick },
        },
  ...
  describe('type attribute', () => {
    ...
    it('should preserve existing type="submit" on native buttons', async () => {
      const container = await render(`<button ngpFocusableWhenDisabled type="submit"></button>`, {
        imports: [NgpFocusableWhenDisabled],
      });
    ...
    it('should preserve existing type="reset" on native buttons', async () => {
      const container = await render(`<button ngpFocusableWhenDisabled type="reset"></button>`, {
        imports: [NgpFocusableWhenDisabled],
      });
  ...
  describe('non-native button keyboard accessibility', () => {
    it('should trigger click on Enter key for non-native buttons', async () => {
      const handleClick = jest.fn();
      const container = await render(`<span ngpFocusableWhenDisabled (click)="onClick()"></span>`, {
        imports: [NgpFocusableWhenDisabled],
        componentProperties: { onClick: handleClick },
      });
    ...
    it('should prevent keyboard activation when disabled with focusableWhenDisabled on non-native', async () => {
      const handleClick = jest.fn();
      const container = await render(
        `<span ngpFocusableWhenDisabled [disabled]="true" [focusableWhenDisabled]="true" (click)="onClick()"></span>`,
        {
          imports: [NgpFocusableWhenDisabled],
          componentProperties: { onClick: handleClick },
        },
  ...
  describe('anchor with empty href', () => {
    it('should treat anchor with empty href as a button (not a valid link)', async () => {
      const container = await render(`<a ngpFocusableWhenDisabled href=""></a>`, {
        imports: [NgpFocusableWhenDisabled],
      });
    ...
    it('should trigger synthetic click on Enter for anchor with empty href', async () => {
      const handleClick = jest.fn();
      const container = await render(
        `<a ngpFocusableWhenDisabled href="" (click)="onClick()"></a>`,
        {
          imports: [NgpFocusableWhenDisabled],
          componentProperties: { onClick: handleClick },
        },
